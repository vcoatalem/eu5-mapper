...
export function CopyrightNotice({ className }: { className?: string }) {
    ...
    return (
      <div className={`text-stone-200 flex flex-col gap-2 text-center font-mono ${className}`}>
        ...
        <p className="text-stone-400 text-sm">EU5 is the property of Paradox Interactive.</p>
        <p className="text-stone-400 text-sm">EU5 Mapper is an unofficial project based on EU5, not affiliated nor endorsed by Paradox Interactive.</p>
        <a className="mt-4 hover:text-blue-500 text-white text-sm" href={`mailto:${email}`}>Contact: {email}</a>
        {modalContext?.close && (
