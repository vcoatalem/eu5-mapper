      ...
      >
        <TooltipTrigger>
          <p className="flex flex-row w-full items-center gap-1 hover:bg-stone-700/50 rounded-md p-1 cursor-help text-sm">
            <b>
              <span className="flex-none w-32">
                {buffDisplayableData.label}
              </span>
            </b>
        ...
        <TooltipContent anchor={{ type: "dom", ref: divRef }}>
          <div className="flex flex-col gap-1 bg-black max-w-96">
            <span>
              <b>{buffDisplayableData.label}</b>:
            </span>
            <span
              dangerouslySetInnerHTML={{
                __html: buffDisplayableData.valueDefinition.description,
              }}
            />
            <span>
              Current value: {sum.toFixed(2)}
              {suffix}
            </span>
            <hr className="w-full border-stone-600 my-1" />
            <span>
              <b>Modifiers Breakdown:</b>
            </span>
            {Object.entries(buffValue)
              .filter(([, v]) => v.value !== 0)
              .map(([name, v]) => (
                <span key={name}>
                  {name}: {v.value.toFixed(2)}
                  {suffix}
                </span>
              ))}
          </div>
