...
function LocationInfoBox(props: {
  ...
  const temporaryData =
    gameState.temporaryLocationData[locationName ?? ""] ?? null;
    ...
    () =>
      locationData
        ? LocationsHelper.getLocationHarborSuitability(
            locationData,
            gameState.ownedLocations[locationData?.name ?? ""],
          )
        : 0,
    [locationData, gameState.ownedLocations],
  );
    ...
    <div className="flex flex-row items-center gap-6 px-4 w-full">
      <div className="flex items-center gap-2 flex-1">
        <span className="font-bold text-base text-white">
          {StringHelper.formatLocationName(locationData.name)}
        </span>
      </div>
      ...
      <div className="flex items-center gap-4 text-sm">
        ...
        {locationData.ownable && (
          <>
            ...
            <span className="flex items-center gap-1">
              <DevelopmentIcon size={24} /> {locationData.development}
            </span>
            <span className="flex items-center gap-1">
              <PopulationIcon size={24} />{" "}
              {NumbersHelper.formatWithSymbol(locationData.population)}
            </span>
        ...
        {locationData.isCoastal && locationData.ownable && (
          <span className="text-white text-md flex flex-items items-center gap-1">
            <HarborSuitabilityIcon size={24} />{" "}
            <span
              style={{
                color: ColorHelper.rgbToHex(
                  ...ColorHelper.getHarborSuitabilityColor(harborCapacity),
                ),
              }}
            >
              {harborCapacity.toFixed(2)}
            </span>
          </span>
        )}
        {(locationData.isSea || locationData.isLake) && (
          <span className="text-white text-md flex items-center gap-1">
            <MaritimePresenceIcon size={24} />{" "}
            <span
              style={{
                color: ColorHelper.rgbToHex(
                  ...ColorHelper.getMaritimePresenceColor(
                    LocationsHelper.getLocationMaritimePresence(
                      locationData,
                      temporaryData,
                    ),
                  ),
      ...
      <span
        className={[
          "ml-auto w-64",
          cta.active ? "text-yellow-500" : "text-stone-400",
        ].join(" ")}
      >
        {cta.label}
      </span>
...
function HierarchyInfoBox(props: { locationNames: ILocationIdentifier[] }) {
  ...
  const cta = useMemo<{ label: string; active: boolean } | null>(() => {
    if (isAcquireMode) {
      return {
        label: `Click to toggle ownership of ${locationsLength} locations`,
        active: true,
      };
    } else {
      return null;
  ...
  const hierarchyTotalPop = useMemo(
    () =>
      props.locationNames.reduce(
        (acc, locationName) =>
          acc +
          LocationsHelper.getLocationPopulation(
            locationName,
    ...
    [props.locationNames, gameData, gameState],
  );
  const hierarchyAverageDevelopment = useMemo(
    () =>
      props.locationNames.reduce(
        (acc, locationName) =>
          acc +
          LocationsHelper.getLocationDevelopment(
            locationName,
            gameData?.locationDataMap ?? {},
            gameState,
          ),
    ...
    [props.locationNames, gameData, gameState, locationsLength],
  );
  ...
  return (
    <div className="flex flex-row items-center gap-6 px-4 w-full">
      <span className="font-bold text-base text-white shrink-0">
        {StringHelper.formatLocationName(hierarchyData)}
      </span>
      <span className="text-stone-400 text-sm max-w-[50%] overflow-hidden text-ellipsis whitespace-nowrap">
        Locations:{" "}
        {props.locationNames
          .map((locationName) => StringHelper.formatLocationName(locationName))
          .join(", ")}
      </span>
  ...
  return (
    <div
      className={[
        ...
        "h-10 bg-black/50 text-stone-400 px-4 flex items-center",
        stripes,
      ].join(" ")}
    >
      {props.children}
...
export function InfoBoxComponent() {
  ...
  if (hoveredLocations.length === 0) {
    return (
      <Container mode={mapEditMode.modeEnabled ?? null}>
  ...
  if (hoveredLocations.length === 1) {
    return (
      <Container mode={mapEditMode.modeEnabled ?? null}>
        <LocationInfoBox
          locationName={hoveredLocations[0]}
          mode={mapEditMode.modeEnabled ?? null}
        ></LocationInfoBox>
      </Container>
    );
  } else {
    return (
      <Container mode={mapEditMode.modeEnabled ?? null}>
        <HierarchyInfoBox locationNames={hoveredLocations}></HierarchyInfoBox>
      </Container>
    );
  }
}
