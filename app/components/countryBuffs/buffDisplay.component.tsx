...
interface IBuffDisplayProps {
  buffKey: keyof typeof countryBuffsMetadata;
  buffValue: number;
  className?: string;
}
...
export function BuffDisplay({
  ...
  return (
    <div
      key={buffKey}
      ref={divRef}
      className={["relative flex flex-row items-center gap-1", className].join(
        " ",
      )}
    >
      <Tooltip
      ...
      >
        <TooltipTrigger>
          <span className="flex-none flex-1 rounded-md p-1 cursor-help text-sma">
            {countryBuffsMetadata[buffKey].label}
          </span>
        </TooltipTrigger>
          ...
          <div className="max-w-96">
            <span
              dangerouslySetInnerHTML={{
                __html:
                  countryBuffsMetadata[buffKey].valueDefinition.description,
              }}
            />
          </div>
        </TooltipContent>
      ...
      <span className="ml-auto">
        {buffValue.toFixed(2)}
        {countryBuffsMetadata[buffKey].valueDefinition.type === "percentage"
          ? "%"
          : ""}
      </span>
    </div>
