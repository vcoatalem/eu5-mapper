...
const DEFAULT_TOOLTIP = "Population";

export function PopulationIcon({ className, size = 24, tooltip }: { className?: string, size?: number, tooltip?: string }) {
  const divRef = useRef<HTMLDivElement>(null);

  return <div className={`cursor-help ${className ?? ""}`.trim()} ref={divRef}>
    <Tooltip>
      <TooltipTrigger>
        <Image src="/gui/icons/population.png" alt="Population" width={size} height={size} />
      </TooltipTrigger>
