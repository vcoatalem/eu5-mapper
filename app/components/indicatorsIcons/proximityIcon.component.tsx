...
const DEFAULT_TOOLTIP = "Proximity";

export function ProximityIcon({ className, size = 24, tooltip }: { className?: string, size?: number, tooltip?: string }) {
  const divRef = useRef<HTMLDivElement>(null);

  return <div className={`cursor-help ${className ?? ""}`.trim()} ref={divRef}>
    <Tooltip>
      <TooltipTrigger>
        <Image src="/gui/icons/proximity.png" alt="Proximity" width={size} height={size} />
      </TooltipTrigger>
